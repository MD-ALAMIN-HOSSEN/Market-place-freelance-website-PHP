

<nav>
    <div class="logo"> <h1>Jobboard</h1> </div>
    <div class="search"> </div>
    <div class="links">
        <a href="../view/dashboard.php" class="button-link">Dashboard</a>
        <a href="../view/jobs.php" class="button-link">jobs</a>
        <a href="../view/gigs.php" class="button-link">gigs</a>
        <a href="../view/orders.php" class="button-link">orders</a>
        <a href="#" class="button-link">messages</a>
        <a href="#" class="button-link">money</a>
        <a href="#" class="button-link">notification</a>
        <a href="../view/profile.php" ><img src="../asset/profile.png" alt="" width="70" height="70"></a>
    </div>
</nav>