...
*/
//this it to load all the jobs
function loadJobs() {
    // Call fetchData ajax for jobs
...
//ajax for all gigs
function fetchDataGigs(callback) {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', '../controller/getGigsJson.php', true);
    //console.log('loded');
    xhr.onload = function() {
        //console.log('loded');
        // Code to handle successful response
...
//this is to load all the gigs
function loadgigs() {
    // Call fetchData ajax
    fetchDataGigs(processDataGigs); //passing a call back funtion whice will render the cards for jobs fron json
}


// for gigs
function processDataGigs(data) {
    console.log(data);
    // Select the container where the cards will be appended
    var container = document.getElementById('card-container');
    ...
    data.forEach(function(item) {
        ...
        var seeMoreLink = document.createElement('a');
        seeMoreLink.classList.add('button-link2', 'space-around');
        seeMoreLink.href = 'gigs_details.php?jobData=' + encodeURIComponent(jobData); // encoding jesone and passing to php file
        seeMoreLink.textContent = 'See more';
